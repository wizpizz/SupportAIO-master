...
namespace SupportAIO.Champions
{
    class Lulu : Champion
    {
            ...
            if (attack != null && attack.IsAlly )
            {
                foreach (var ally in GameObjects.AllyHeroes.Where(
                        x => x.Distance(Player) <= W.Range && x.IsAlly && !x.IsRecalling() &&
                             RootMenu["combo"]["wset"]["ally"][x.CharacterName.ToLower() + "priority"].GetValue<MenuSlider>().Value != 0)
                    .OrderByDescending(
                        x => RootMenu["combo"]["wset"]["ally"][x.CharacterName.ToLower() + "priority"].GetValue<MenuSlider>().Value))
                {
        ...
        protected override void Combo()
        {
            ...
            bool useW = RootMenu["combo"]["wset"]["usew"].GetValue<MenuBool>().Enabled;
            bool useR = RootMenu["combo"]["rset"]["user"].GetValue<MenuBool>().Enabled;
            bool useEQ = RootMenu["combo"]["useeq"].GetValue<MenuBool>().Enabled;
            if (useEQ)
            {
        ...
        protected override void Drawings()
        {
            ...
            if (RootMenu["drawings"]["drawq"].GetValue<MenuBool>().Enabled)
            {
                Render.Circle.DrawCircle(Player.Position, Q.Range, Color.Crimson);
            }
            if (RootMenu["drawings"]["draww"].GetValue<MenuBool>().Enabled)
            {
            ...
            if (RootMenu["drawings"]["drawpix"].GetValue<MenuBool>().Enabled)
            {
            ...
            if (RootMenu["drawings"]["pixranges"].GetValue<MenuBool>().Enabled)
            {
                foreach (var pixs in GameObjects.AllGameObjects)
                {
                    if (pixs.Name == "RobotBuddy" && pixs.IsValid && pixs != null && !pixs.IsDead && pixs.Team == Player.Team)
        ...
        protected override void Killsteal()
        {
            ...
            if (E.IsReady() &&
                RootMenu["killsteal"]["kse"].GetValue<MenuBool>().Enabled)
            {
            ...
            if (Q.IsReady() && RootMenu["killsteal"]["kseq"].GetValue<MenuBool>().Enabled)
            {
        ...
        protected override void Harass()
        {
            ...
            bool useEQ = RootMenu["harass"]["useeq"].GetValue<MenuBool>().Enabled;
            if (useEQ)
            {
        ...
        internal override void OnGapcloser(AIHeroClient target, AntiGapcloser.GapcloserArgs args)
        {
        ...
        protected override void SetSpells()
        {
            ...
            Q.SetSkillshot(0.25f, 60, 1400, false, SpellType.Line,  HitChance.None);
        }
    }
}
